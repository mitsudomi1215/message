...
(function() {
    ...
    let LookUpSample = (function(fieldSettings){
      ...
      LookUpSample.prototype = {
        ...
        showModal: function(){
          ...
          companyCode.setAttribute("placeholder", "グループカテゴリ");
          ...
          this.getButton.addEventListener('click', function(){
            ...
            if (fullRecords.length > 0) {
                ...
                for (let i = 0; i < fullRecords.length; i++) {
                  ...
                  if (companyName) {
                    if (fullRecords[i].本部関連_商品関連の宛先を取得1.value.indexOf(companyName) === -1) { //あいまい検索するためindexOfで評価する
        ...
        copyDatas: function(modalRecord){
          ...
          this.event.record[this.fieldSettings.recordIdField].value = modalRecord.本部関連_商品関連の宛先を取得1.value;
    ...
    let lookUpParams = {
      ...
      buttonSpace: 'irregular_lookup_button1', //ボタン設置用のスペースフィールド
      recordIdField: '本部関連_商品関連の宛先を取得1', //ルックアップ先のレコード番号保存用のフィールド
      app: lookupAppId,
      copyField: {
        to: '本部関連_商品関連の宛先を取得1', //自作のルックアップフィールド
        from: 'グループカテゴリ' //コピー元のフィールド
      },
      //コピー先のフィールドを追加したい場合は以下に追加していく
      otherCopyFields: [ //ほかのフィールドのコピー
      {to: '本部関連_商品関連の宛先を取得1', from: 'グループカテゴリ'},//fromが取得先アプリのフィールドコード
      {to: '_1権限追加ユーザー1', from: 'ユーザー1'},    //fromが取得先アプリのフィールドコード
    //   {to: '本部', from: '本部'},//変更
    //   {to: '部', from: '部'},//変更
    //   {to: 'エリア', from: 'エリア'},//変更
    //   {to: '業態', from: '業態'},//変更
    //   {to: '区分', from: '区分'},//変更
      ...
      viewFields: ['グループカテゴリ'], //modalに表示するフィールド
      companyNameFieldName : '本部関連_商品関連の宛先を取得1',
    ...
    async function fetchAllRecords(maxRecordNumber, records, companyName, tantouName){
        ...
        let params = {
            ...
            'query' : ' レコード番号 > ' + maxRecordNumber + ' order by レコード asc limit ' + searchLimit + ' ', // 全量取得するためにレコード番号でソートした上で絞り込んでいく
            'totalCount' : true
        };
        ...
        return await kintone.api(kintone.api.url('/k/v1/records', true), 'GET', params).then(function(response){
            if (response.records.length > 0) {
                for (let i = 0; i < response.records.length; i++) {
                  ...
                  let isCompanyNameExist = true;
                  if (companyName) {
                    if (response.records[i].グループカテゴリ.value.indexOf(companyName) === -1) { //あいまい検索するためindexOfで評価する
                      isCompanyNameExist = false;
                    }
                  }
