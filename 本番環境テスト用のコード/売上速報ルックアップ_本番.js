...
// ヘッダーここまで-------------------------------------------------------------------------
  ...
  let lookUpParams = {
    ...
    {to: '時間数_計画' , from: '人時単日_人時数計画'},
    {to: '時間数_週マネ' , from: '週マネ単日_人時数実績'},
    {to: '時間数_実績' , from: '人時単日_人時数実績'},
    {to: '時間数_要コン' , from: '人時単日_要コン'},
    {to: '欠員', from: '単日_欠員'}
    ],
    viewFields: ['店番', '店名','日付', '売上単日_計画','週マネ単日_売上見込','売上単日_実績','売上単日_達成率','人時単日_人時数計画','週マネ単日_人時数実績','人時単日_人時数実績','人時単日_要コン','単日_欠員'], //modalに表示するフィールド//変更
  ...
  async function fetchAllRecords(maxRecordNumber, records, companyName, tantouName,event){
      ...
      let params = {
        ...
        'query': '店番="' + event.record.店番.value + '" and 日付 > "' + getThreeDaysBefore(event.record.利用日_まとめ.value) + '" and 日付 < "' + getThreeDaysAfter(event.record.利用日_まとめ.value) + '" order by 日付 desc limit ' + searchLimit,
        'totalCount': true
    };
